...
public class JSONObject
{
    ...
    public BigInteger getBigInteger(final String key) throws JSONException {
        ...
        throw new JSONException("JSONObject[" + quote(key) + "] could not be converted to BigInteger (" + object + ").");
    }
    ...
    public BigDecimal getBigDecimal(final String key) throws JSONException {
        ...
        throw new JSONException("JSONObject[" + quote(key) + "] could not be converted to BigDecimal (" + object + ").");
    }
    ...
    private void populateMap(final Object bean) {
        ...
        for (final Method method : array) {
            ...
            if (Modifier.isPublic(modifiers) && !Modifier.isStatic(modifiers) && method.getParameterTypes().length == 0 && !method.isBridge() && method.getReturnType() != Void.TYPE && this.isValidMethodName(method.getName())) {
                final String key = this.getKeyNameFromMethod(method);
    ...
    private boolean isValidMethodName(final String name) {
